...
| `probe.attribute_ds_queries` | Off by default. When `true` (requires `metrics.prometheus_url`), an extra probe pass replays a small distinct batch per relation, one relation at a time, and diffs OpenFGA's `openfga_datastore_query_count` to attribute **datastore queries per check per relation** — the sharpest signal for spotting an expensive rewrite. Adds a "DS queries/check (probe)" column to the per-relation table. Best-effort; when off the probe path is byte-identical. |
| `load.endpoint` | [`check`](https://openfga.dev/docs/interacting/relationship-queries#check) (one tuple per request), [`batch-check`](https://openfga.dev/docs/interacting/relationship-queries#batch-check) (many per request), [`list-objects`](https://openfga.dev/docs/interacting/relationship-queries#listobjects) ("which objects can this user access?"), or [`list-users`](https://openfga.dev/docs/interacting/relationship-queries#listusers) ("who can access this object?"). The list endpoints reuse the corpus subjects/relations and report a result-set-size distribution; their result verification is a best-effort spot-check. Also accepts a **weighted blend** — `endpoint: {check: 70, list-objects: 20, batch-check: 10}` — so one run measures the contention real services see when several endpoints share the server; each request picks an endpoint by weight and the findings split percentiles per endpoint. |
...
| Per-endpoint breakdown | One row per endpoint with its configured share and latency percentiles. Only present for a mixed-endpoint run (`load.endpoint` as a weighted blend) — use it to see whether one endpoint's load degraded another's. |
| Result-set sizes | Distribution of returned-set sizes (mean/p50/p99/max, empty-response rate). Only present for `list-objects`/`list-users` runs — the size of the result set is the headline cost driver for those endpoints. |
...
| Per-relation breakdown | The most useful place to compare similar paths, including per-relation error counts. Gains a "DS queries/check (probe)" column when `probe.attribute_ds_queries` ran, attributing datastore reads per relation. |
